...
#
# Launcher (DEV mode — Vite dev server + Electron with HMR). All launch logic
# lives here in PowerShell because cmd.exe cannot reliably parse a long batch
# file containing multi-byte (e.g. 中文) characters — its UTF-8 buffer handling
# splits lines mid-token. start_dev.cmd is a thin ASCII wrapper around this file.
param(
    [string]$L,
    [string]$R
)
...
Write-Host '========================================'
Write-Host '  M2_GIT_DIFF - Launcher (DEV MODE)'
Write-Host '========================================'
Write-Host ''
...
# --- 觸發程式執行 (Vite dev server + Electron, 含 HMR) ---
Write-Host '[啟動] 開始執行程式 (DEV MODE)...'
Write-Host ''
npm run dev
exit $LASTEXITCODE
