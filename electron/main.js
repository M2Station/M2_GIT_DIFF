...
ipcMain.handle('vscode:chat', async (_evt, payload) => {
  ...
  // Write the prompt to a temp file and feed it to `code chat -` via shell
  // redirection instead of piping through Node's stdin. Piping stdin into the
  // Windows `code.cmd` batch shim is racy — ending the pipe right after spawn
  // can drop the prompt before the shim attaches its reader, so VS Code opens
  // but the chat receives nothing. Redirection lets cmd.exe read the file
  // directly and removes that race. The temp path is generated by us (no user
  // content on the command line), so the injection-safe design is preserved.
  ...
  try {
    ...
    throw new Error('Failed to prepare chat prompt: ' + e.message);
  }
  const cleanup = () => {
    try {
  ...
  return await new Promise((resolve, reject) => {
    ...
    try {
      ...
      // no user content.
      const cmdLine = `"${codeCmd}" chat -r -m ${chatMode} - < "${tmpFile}"`;
    ...
    // Remove the temp file once cmd.exe has finished reading it.
