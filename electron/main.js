...
// Build the English context document handed to VS Code chat for a commit:
// metadata plus the full `git show` diff, so the chat session is self-contained
// even in a new (folder-less) window. Kept entirely in English by design.
function buildCommitChatContext(repoPath, commit) {
  const lines = [
    `# Commit ${commit.short || commit.sha} - ${commit.subject || ''}`.trimEnd(),
    '',
    ...
    `Date: ${commit.authorDate || '(unknown)'}`,
    '',
    '## Commit message',
    commit.body && commit.body.trim() ? commit.body : '(no body)',
    '',
    '## Full diff (git show)'
  ];
  ...
  if (repoPath && fs.existsSync(repoPath)) {
    ...
    diff = '(repo path unavailable)';
  }
  lines.push(diff.trim() || '(empty diff)');
  return lines.join('\n');
}

// Remove our own stale chat-context temp files so they don't accumulate. We
// cannot delete a file right after launch because `--add-file` is read by VS
// Code only when the user submits their prompt (which can be much later), so
// instead we sweep files older than a generous window on each invocation.
function sweepStaleChatTemps() {
  ...
  try {
    for (const name of fs.readdirSync(dir)) {
      if (!/^m2gd-chat-[0-9a-f]+\.txt$/.test(name)) continue;
...
// Open VS Code chat in a NEW session with the commit loaded as context, then
// let the user type their own prompt to trigger the chat. We deliberately do
// NOT pass a prompt: `code chat <prompt>`/stdin submits and runs immediately,
// whereas `--add-file` with no prompt attaches the context and waits for the
// user. `-n` forces a fresh session. Every command-line token is constant,
// whitelisted, or a path we generated, so there is no shell-injection surface.
ipcMain.handle('vscode:chat', async (_evt, payload) => {
  ...
  if (!commit || typeof commit.sha !== 'string' || !commit.sha) {
    throw new Error('commit with a sha is required');
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    try {
      ...
      // no user content. No prompt is passed, so the session waits for the
      // user instead of auto-running.
